...
class Board:
    ...
    def __init__(self, start_type):
        ...
        self.heights = [[0 for _ in range(COLS)] for _ in range(ROWS)]
    ...
    def create_board(self):
        ...
        self.board[worker_two[0]][worker_two[1]] = (Worker(worker_two, PLAYER_TWO, 1))
    ...
    def draw(self, win):
        ...
        for row in range(ROWS):
            for col in range(COLS):
                ...
                # Vẽ tất cả các tầng từ 1 đến height (nếu có)
                for level in range(1, height + 1):
                    building = Building([row, col], level)
                    building.draw(win)

                # Vẽ worker sau cùng (nếu có)
                if isinstance(item, Worker):
                    item.height = height  # cập nhật chiều cao hiện tại
                    item.on_building = height > 0
                    item.draw(win)
    ...
    def possible_moves(self, worker):
        ...
        # else:
        #     # Filter moves to ensure the worker can move to a building that is either the same height or one height-unit higher
        for move in self.buildings:
            row, col = move
